...
/*   Updated: 2019/04/08 15:12:14 by mdchane          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_lpath	*find_biggest(t_lpath *tab)
{
	...
	while (tab)
	{
		if (!tab->removed && max->len < tab->len)
			max = tab;
		tab = tab->next;
	}
	tab = beg;
...
void	remove_path(t_lpath *tab, int start_ants, t_pack *pack)
{
	t_lpath	*beg;
	int		count;
	t_lpath	*max;
